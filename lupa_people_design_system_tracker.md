...
> Companion to `lupa_people_roadmap_v2.md` and `lupa_people_user_stories.md`.
...
| `KPIRow` / `CompetencyRow` | ☐ | builder rows (name, definition, kind/observable, outcomes/impact) |
...
| `Scorecard` | ☐ | shared Lupa Way block + role-KPI block, made of `ScorecardRow`s |
| `FrameworkBuilder` | ☐ | Lupa Way (Sections→Competencies) **and** role-KPI variants; versioned, publish action |
...
| `DashboardCard` | ☐ | KPI/metric card for Phase 4 |
...
| Screen | Status | Notes |
|---|---|---|
| Lupa Way builder (admin) | ☐ | Sections→Competencies, versioned |
| Role-KPI builder (manager) | ☐ | per job title, versioned |
...
- **`FrameworkBuilder` is reused** for both Lupa Way and role-KPIs — build once with a variant, not twice.
